...
Description:
    Predicts future daily K-line (1D) data for A-share markets using Conductor model and akshare.
...
# Setting
TOKENIZER_PRETRAINED = "NeoQuasar/Conductor-Tokenizer-base"
MODEL_PRETRAINED = "NeoQuasar/Conductor-base"
DEVICE = "cpu"  # "cuda:0"
MAX_CONTEXT = 512
LOOKBACK = 400
PRED_LEN = 120
...
def plot_result(df_hist, df_pred, symbol):
    ...
    plt.title(f"Conductor Prediction for {symbol}")
    plt.xlabel("Date")
...
def predict_future(symbol):
    print(f"🚀 Loading Conductor tokenizer:{TOKENIZER_PRETRAINED} model:{MODEL_PRETRAINED} ...")
    tokenizer = ConductorTokenizer.from_pretrained(TOKENIZER_PRETRAINED)
    model = Conductor.from_pretrained(MODEL_PRETRAINED)
    predictor = ConductorPredictor(model, tokenizer, device=DEVICE, max_context=MAX_CONTEXT)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Conductor stock prediction script")
