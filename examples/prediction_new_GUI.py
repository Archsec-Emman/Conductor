...
try:
    ...
    print("⚠️ 无法导入Conductor模型，预测功能将不可用")
...
class StockPredictorGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Conductor股票预测系统")
        ...
        self.default_config = {
            "stock_code": "600580",
            "stock_name": "卧龙电驱",
            "data_dir": r"D:\lianghuajiaoyi\Conductor\examples\data",
            "output_dir": r"D:\lianghuajiaoyi\Conductor\examples\yuce",
            "pred_days": 60,
            "history_years": 1
        }
    ...
    def create_widgets(self):
        ...
        desc_label = tk.Label(
            self.root,
            text="基于Conductor模型的多维度股票价格预测系统",
            font=("Arial", 10),
            bg='#f0f0f0',
            fg='#7f8c8d'
        )
        ...
        tk.Label(dir_frame, text="数据目录:", bg='#f0f0f0', font=("Arial", 10)).grid(row=0, column=0, sticky=tk.W,
                                                                                     padx=5, pady=5)
        self.data_dir_var = tk.StringVar(value=r"D:\lianghuajiaoyi\Conductor\examples\data")
        ...
        tk.Label(dir_frame, text="输出目录:", bg='#f0f0f0', font=("Arial", 10)).grid(row=1, column=0, sticky=tk.W,
                                                                                     padx=5, pady=5)
        self.output_dir_var = tk.StringVar(value=r"D:\lianghuajiaoyi\Conductor\examples\yuce")
    ...
    def reset_fields(self):
        ...
        self.data_dir_var.set(r"D:\lianghuajiaoyi\Conductor\examples\data")
        self.output_dir_var.set(r"D:\lianghuajiaoyi\Conductor\examples\yuce")
...
def run_comprehensive_prediction_gui(stock_code, stock_name, data_dir, pred_days, output_dir, history_years=1,
    ...
    try:
        ...
        # 2. 加载模型和分词器
        update_progress("\n步骤2: 加载Conductor模型和分词器...")
        try:
            tokenizer = ConductorTokenizer.from_pretrained("NeoQuasar/Conductor-Tokenizer-base")
            model = Conductor.from_pretrained("NeoQuasar/Conductor-base")
            update_progress("✅ 模型加载完成 - 使用Conductor-base模型")
        except Exception as e:
        ...
        predictor = ConductorPredictor(model, tokenizer, device="cuda:0", max_context=512)
