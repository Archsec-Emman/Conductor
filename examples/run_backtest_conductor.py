...
class ConductorBacktester:
    ...
    def load_predictions(self, stock_code):
        ...
        # 尝试不同的预测文件命名
        pred_files = [
            os.path.join(self.model_dir, f"{stock_code}_conductor_predictions.csv"),
    ...
    def plot_backtest_results(self, backtest_results, metrics, stock_code, output_dir):
        ...
        ax1.set_title(f'{stock_code} Conductor模型回测结果', fontsize=14, fontweight='bold')
...
def main():
    ...
    BACKTEST_CONFIG = {
        ...
        "data_dir": r"D:\lianghuajiaoyi\Conductor\examples\data",  # 历史数据目录
        "model_dir": r"D:\lianghuajiaoyi\Conductor\examples\yuce",  # 模型预测结果目录
        "output_dir": r"D:\lianghuajiaoyi\Conductor\examples\backtest",  # 回测结果输出目录
        "initial_capital": 100000,  # 初始资金
        "threshold": 0.02  # 交易阈值（2%）
    }

    print("🤖 Conductor模型回测系统")
    print("=" * 50)
    ...
    # 创建回测器并运行
    backtester = ConductorBacktester(
