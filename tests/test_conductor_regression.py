...
from model import Conductor, ConductorPredictor, ConductorTokenizer
...
def test_conductor_predictor_regression(context_len):
    set_seed(SEED)
    ...
    tokenizer = ConductorTokenizer.from_pretrained("NeoQuasar/Conductor-Tokenizer-base", revision=TOKENIZER_REVISION)
    model = Conductor.from_pretrained("NeoQuasar/Conductor-small", revision=MODEL_REVISION)
    tokenizer.eval()
    model.eval()

    predictor = ConductorPredictor(model, tokenizer, device=DEVICE, max_context=MAX_CTX_LEN)
...
def test_conductor_predictor_mse(context_len, expected_mse):
    set_seed(SEED)
