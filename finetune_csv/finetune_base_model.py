...
def main():
    ...
    parser = argparse.ArgumentParser(description='Conductor Basemodel Fine-tuning Training')
    ...
    if getattr(config, 'pre_trained_tokenizer', True):
        tokenizer = ConductorTokenizer.from_pretrained(config.finetuned_tokenizer_path)
    else:
        import json, os
    ...
    if getattr(config, 'pre_trained_predictor', True):
        model = Conductor.from_pretrained(config.pretrained_predictor_path)
    else:
        import json, os
