...
class SequentialTrainer:
    ...
    def train_tokenizer_phase(self):
        ...
        if getattr(self.config, 'pre_trained_tokenizer', True):
            ...
            tokenizer = ConductorTokenizer.from_pretrained(self.config.pretrained_tokenizer_path)
        else:
            if self.rank == 0:
    ...
    def train_basemodel_phase(self):
        ...
        if getattr(self.config, 'pre_trained_tokenizer', True):
            ...
            tokenizer = ConductorTokenizer.from_pretrained(self.config.finetuned_tokenizer_path)
        else:
            if self.rank == 0:
        ...
        if getattr(self.config, 'pre_trained_predictor', True):
            ...
            model = Conductor.from_pretrained(self.config.pretrained_predictor_path)
        else:
            if self.rank == 0:
    ...
    def run_training(self):
        if self.rank == 0:
            print("Starting Conductor model sequential fine-tuning training")
...
def main():
    parser = argparse.ArgumentParser(description='Conductor Model Sequential Fine-tuning Training')
