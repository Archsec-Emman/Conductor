...
try:
    ...
    print("Warning: Conductor model cannot be imported, will use simulated data for demonstration")
...
AVAILABLE_MODELS = {
    'conductor-mini': {
        'name': 'Conductor-mini',
        'model_id': 'NeoQuasar/Conductor-mini',
        'tokenizer_id': 'NeoQuasar/Conductor-Tokenizer-2k',
        'context_length': 2048,
        'params': '4.1M',
        'description': 'Lightweight model, suitable for fast prediction'
    },
    'conductor-small': {
        'name': 'Conductor-small',
        'model_id': 'NeoQuasar/Conductor-small',
        'tokenizer_id': 'NeoQuasar/Conductor-Tokenizer-base',
        'context_length': 512,
        'params': '24.7M',
    ...
    'conductor-base': {
        'name': 'Conductor-base',
        'model_id': 'NeoQuasar/Conductor-base',
        'tokenizer_id': 'NeoQuasar/Conductor-Tokenizer-base',
        'context_length': 512,
        'params': '102.3M',
...
def create_prediction_chart(df, pred_df, lookback, pred_len, actual_df=None, historical_start_idx=0):
    ...
    # Update layout
    fig.update_layout(
        title='Conductor Financial Prediction Results - 400 Historical Points + 120 Prediction Points vs 120 Actual Points',
        xaxis_title='Time',
        yaxis_title='Price',
        ...
        if MODEL_AVAILABLE and predictor is not None:
            try:
                ...
                if start_date:
                    ...
                    prediction_type = f"Conductor model prediction (within selected window: first {lookback} data points for prediction, last {pred_len} data points for comparison, time span: {time_span})"
                else:
                    # Use latest data
                    x_df = df.iloc[:lookback][required_cols]
                    x_timestamp = df.iloc[:lookback]['timestamps']
                    y_timestamp = df.iloc[lookback:lookback+pred_len]['timestamps']
                    prediction_type = "Conductor model prediction (latest data)"
                
                # Ensure timestamps are Series format, not DatetimeIndex, to avoid .dt attribute error in Conductor model
                ...
                return jsonify({'error': f'Conductor model prediction failed: {str(e)}'}), 500
        else:
            return jsonify({'error': 'Conductor model not loaded, please load model first'}), 400
...
def load_model():
    """Load Conductor model"""
    global tokenizer, model, predictor
    
    try:
        if not MODEL_AVAILABLE:
            return jsonify({'error': 'Conductor model library not available'}), 400
        
        data = request.get_json()
        model_key = data.get('model_key', 'conductor-small')
        ...
        tokenizer = ConductorTokenizer.from_pretrained(model_config['tokenizer_id'])
        model = Conductor.from_pretrained(model_config['model_id'])
        
        # Create predictor
        predictor = ConductorPredictor(model, tokenizer, device=device, max_context=model_config['context_length'])
    ...
    if MODEL_AVAILABLE:
        if predictor is not None:
            return jsonify({
                'available': True,
                'loaded': True,
                'message': 'Conductor model loaded and available',
                'current_model': {
            ...
            return jsonify({
                'available': True,
                'loaded': False,
                'message': 'Conductor model available but not loaded'
            })
    else:
        return jsonify({
            'available': False,
            'loaded': False,
            'message': 'Conductor model library not available, please install related dependencies'
        })

if __name__ == '__main__':
    print("Starting Conductor Web UI...")
    print(f"Model availability: {MODEL_AVAILABLE}")
    if MODEL_AVAILABLE:
        print("Tip: You can load Conductor model through /api/load-model endpoint")
    else:
