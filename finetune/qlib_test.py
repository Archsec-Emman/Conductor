...
from model.conductor import Conductor, ConductorTokenizer, auto_regressive_inference
...
def load_models(config: dict) -> tuple[ConductorTokenizer, Conductor]:
    """Loads the fine-tuned tokenizer and predictor model."""
    device = torch.device(config['device'])
    print(f"Loading models onto device: {device}...")
    tokenizer = ConductorTokenizer.from_pretrained(config['tokenizer_path']).to(device).eval()
    model = Conductor.from_pretrained(config['model_path']).to(device).eval()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Conductor Inference and Backtesting")
